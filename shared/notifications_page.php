...
$profileLink = $role === 'client' ? '/jobhub/client/profile.php' : '/jobhub/freelancer/profile.php';
?>
<!DOCTYPE html>
    ...
    <nav class="fixed top-0 inset-x-0 z-50 py-3 bg-white/80 backdrop-blur-md border-b border-gray-100 dark:bg-slate-900/80 dark:border-slate-700">
        <div class="w-full mx-auto px-4 sm:px-6 flex items-center justify-between">
            <div class="flex items-center gap-6">
                <a href="/jobhub/index.php" class="flex items-center gap-1.5 group shrink-0">
                    <img src="/jobhub/assets/upload/logos/logo.png" alt="Logo" class="w-[36px] h-[36px] rounded-xl">
                ...
                <a href="<?= $role === 'client' ? '/jobhub/client/dashboard.php' : '/jobhub/freelancer/home.php' ?>" class="text-sm font-semibold text-gray-600 dark:text-slate-400 hover:text-gray-900 dark:hover:text-white transition">
                    <i data-lucide="arrow-left" class="mr-1"></i> Back to Dashboard
                </a>
            </div>
    ...
    <div class="pt-20 pb-10 px-4 sm:px-6 max-w-3xl mx-auto">
            ...
            <?php if ($unreadCount > 0): ?>
                <form method="POST" class="inline">
                    ...
                    <button type="submit" class="text-sm font-semibold text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition">
                        <i data-lucide="circle-check" class="mr-1"></i> Mark all read
                    </button>
                </form>
            <?php endif; ?>
        </div>
        ...
        <?php if (empty($notifications)): ?>
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-12 text-center">
                <i data-lucide="bell-off" class="text-4xl text-gray-300 dark:text-slate-600 mb-4 block"></i>
        ...
        <?php else: ?>
            <div class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 overflow-hidden divide-y divide-gray-50 dark:divide-slate-700">
                <?php foreach ($notifications as $n): ?>
                    ...
                    $iconMap = [
                        'new_proposal' => ['icon' => 'file-text', 'bg' => 'bg-blue-100', 'color' => 'text-blue-600'],
                        'proposal_accepted' => ['icon' => 'circle-check', 'bg' => 'bg-green-100', 'color' => 'text-green-600'],
                        'new_contract' => ['icon' => 'handshake', 'bg' => 'bg-purple-100', 'color' => 'text-purple-600'],
                        'milestone_submitted' => ['icon' => 'flag', 'bg' => 'bg-yellow-100', 'color' => 'text-yellow-600'],
                        'payment_released' => ['icon' => 'banknote', 'bg' => 'bg-emerald-100', 'color' => 'text-emerald-600'],
                        'new_message' => ['icon' => 'message-circle', 'bg' => 'bg-cyan-100', 'color' => 'text-cyan-600'],
                        'review_received' => ['icon' => 'star', 'bg' => 'bg-orange-100', 'color' => 'text-orange-600'],
                        'job_invitation' => ['icon' => 'send', 'bg' => 'bg-indigo-100', 'color' => 'text-indigo-600'],
                        'invitation_accepted' => ['icon' => 'thumbs-up', 'bg' => 'bg-green-100', 'color' => 'text-green-600'],
                        'invitation_declined' => ['icon' => 'thumbs-down', 'bg' => 'bg-red-100', 'color' => 'text-red-600'],
                    ];
                    $iconData = $iconMap[$n['type']] ?? ['icon' => 'bell', 'bg' => 'bg-gray-100', 'color' => 'text-gray-600'];
                    $link = $n['link'] ?? '#';
                    ?>
                    <a href="<?= htmlspecialchars($link) ?>" class="flex items-start gap-4 px-5 py-4 hover:bg-gray-50 dark:hover:bg-slate-700/50 transition <?= $isUnread ? 'bg-blue-50/40 dark:bg-blue-900/10' : '' ?>">
                        <div class="flex-shrink-0 mt-0.5 w-10 h-10 rounded-full flex items-center justify-center <?= $iconData['bg'] ?>">
                            <i data-lucide="<?= $iconData['icon'] ?>" class="text-sm <?= $iconData['color'] ?>"></i>
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[11px] text-gray-400 dark:text-slate-500 mt-1.5">
                                <i data-lucide="clock" class="mr-1"></i><?= htmlspecialchars($n['created_at']) ?>
                            </p>
                        </div>
                        <?php if ($isUnread): ?>
