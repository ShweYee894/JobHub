 ...
 * Shows all disputes raised by or against the client.
 */
...
$querySql = "SELECT d.id, d.raised_by, d.reason, d.description, d.status, d.resolution, d.created_at, d.updated_at,
                    ...
                    CASE WHEN d.raised_by = ? THEN 'client' ELSE 'freelancer' END AS raised_by_role,
...
$uStmt->close();
$user = ['name' => $_uRow['name'] ?? 'Client', 'profile_image' => $_uRow['profile_image'] ?? null];
...
<main class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
                    ...
                    <div class="p-6">
                        ...
                        <div class="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-slate-400 mb-4">
                            ...
                            <?php if (!empty($d['milestone_title'])): ?>
                                <span class="flex items-center gap-1.5">
                                    <i data-lucide="list-checks" class="w-4 h-4 text-violet-500"></i>
                                    <?= sanitize_string($d['milestone_title']) ?>
                                    <span class="font-bold text-gray-700 dark:text-slate-300">(<?= format_currency((float) $d['milestone_amount']) ?>)</span>
                                </span>
                            <?php endif; ?>
                        ...
                        <!-- Reason & Description -->
                        <div class="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4 border border-gray-100 dark:border-slate-600 mb-4">
                            <div class="flex items-center gap-2 mb-2">
                                <span class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Reason</span>
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold bg-gray-100 dark:bg-slate-600 text-gray-700 dark:text-gray-300">
                                    <?php
                                    $reasonKey = $d['reason'];
                        ...
                        <?php if (!empty($d['milestone_title'])): ?>
                            <div class="flex items-center gap-2 mb-4 text-xs">
                                ...
                                <span class="inline-flex items-center px-2 py-0.5 rounded font-semibold <?= $d['milestone_status'] === 'disputed' ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-gray-100 text-gray-600 border border-gray-200' ?>">
                        ...
                        <?php if (!empty($d['resolution'])): ?>
                            <div class="bg-emerald-50/50 dark:bg-emerald-900/10 rounded-xl p-4 border border-emerald-200 dark:border-emerald-800/30">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <?php if (!empty($d['resolved_by_name'])): ?>
                                        <span class="text-[10px] text-emerald-500 dark:text-emerald-400">by <?= sanitize_string($d['resolved_by_name']) ?></span>
                                    <?php endif; ?>
                                </div>
                                <p class="text-sm text-gray-700 dark:text-slate-300 leading-relaxed"><?= nl2br(sanitize_string($d['resolution'])) ?></p>
    ...
    <?php else: ?>
        <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-sm p-12 text-center">
            ...
            <p class="text-gray-400 dark:text-slate-500 text-xs">Disputes will appear here when you or a freelancer raises one</p>
        </div>
    <?php endif; ?>
    <?php $conn->close(); ?>
</main>
