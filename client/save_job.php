...
if ($budget <= 0) $errors[] = 'Budget must be a valid amount greater than 0.';
...
try {
    ...
    if ($jobId > 0) {
        ...
        $stmt = $conn->prepare("UPDATE jobs SET title = ?, description = ?, budget = ?, embedding_vector = ?, updated_at = NOW() WHERE id = ? AND client_id = ?");
        $stmt->bind_param('ssdsii', $title, $description, $budget, $embeddingJson, $jobId, $clientId);
        $stmt->execute();
        $stmt->close();
        ...
        $stmt = $conn->prepare("INSERT INTO jobs (client_id, title, description, budget, status, embedding_vector, created_at, updated_at) VALUES (?, ?, ?, ?, 'open', ?, NOW(), NOW())");
        $stmt->bind_param('issds', $clientId, $title, $description, $budget, $embeddingJson);
        $stmt->execute();
