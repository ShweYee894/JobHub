...
<?php endif; ?>
    <form action="save_job.php" method="POST" id="job-form" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-gray-100 shadow-sm fade-in dark:bg-gray-800 dark:border-gray-700">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center dark:bg-blue-900/30"><i class="fas fa-file-lines text-blue-500"></i></div>
                <div>
                    <h2 class="text-base font-bold text-gray-900 dark:text-white">Job Information</h2>
                    <p class="text-xs text-gray-400">Provide a clear title and detailed description</p>
                </div>
            </div>
            <div class="space-y-5">
                <div>
                    <label for="title" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Job Title <span class="text-red-500">*</span></label>
                    <input type="text" id="title" name="title" required value="<?= sanitize_string($prefill['title']) ?>" placeholder="e.g. Build a responsive e-commerce website" maxlength="255" class="fld w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400">
                    <p id="title-err" class="text-red-500 text-xs mt-1.5 hidden"></p>
                </div>
                <div>
                    <label for="description" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Job Description <span class="text-red-500">*</span></label>
                    <textarea id="description" name="description" rows="8" required placeholder="Describe your project in detail..." class="fld w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-gray-900 placeholder-gray-400 text-sm resize-none dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400"><?= sanitize_string($prefill['description']) ?></textarea>
                    <p id="description-err" class="text-red-500 text-xs mt-1.5 hidden"></p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-gray-100 shadow-sm fade-in dark:bg-gray-800 dark:border-gray-700" style="animation-delay:.1s">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-10 h-10 rounded-xl bg-emerald-50 flex items-center justify-center dark:bg-emerald-900/30"><i class="fas fa-dollar-sign text-emerald-500"></i></div>
                <div>
                    <h2 class="text-base font-bold text-gray-900 dark:text-white">Budget</h2>
                    <p class="text-xs text-gray-400">Set a fixed budget for this project</p>
                </div>
            </div>
            <div class="max-w-sm">
                <label for="budget" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">Budget Amount (USD) <span class="text-red-500">*</span></label>
                <div class="relative">
                    <div class="absolute left-4 top-1/2 -translate-y-1/2 flex items-center justify-center w-6 h-6 rounded-lg bg-emerald-100 dark:bg-emerald-900/50"><i class="fas fa-dollar-sign text-emerald-600 text-xs"></i></div>
                    <input type="number" id="budget" name="budget" required step="0.01" min="0.01" value="<?= sanitize_string($prefill['budget']) ?>" placeholder="0.00" class="fld w-full bg-gray-50 border border-gray-200 rounded-xl pl-12 pr-4 py-3 text-gray-900 placeholder-gray-400 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400">
                </div>
                <p id="budget-err" class="text-red-500 text-xs mt-1.5 hidden"></p>
            </div>
        </div>
        <div class="bg-white rounded-2xl p-6 sm:p-8 border border-gray-100 shadow-sm fade-in dark:bg-gray-800 dark:border-gray-700" style="animation-delay:.2s">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-gray-50 pb-5 dark:border-gray-700">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center text-blue-600 dark:bg-blue-900/30"><i class="fas fa-layer-group text-lg"></i></div>
                    <div>
                        <h2 class="text-base font-bold text-gray-900 dark:text-white">Required Skills</h2>
                        <p class="text-xs text-gray-400">Select the skills needed for this project.</p>
                    </div>
                </div>
                <span id="skill-count" class="bg-gray-100 text-gray-400 text-xs font-semibold px-3 py-1.5 rounded-full border border-gray-200/60 transition-all dark:bg-gray-700 dark:text-gray-400 dark:border-gray-600">0 Selected</span>
            </div>
            <div class="relative mb-6">
                <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm"></i>
                <input type="text" id="skill-search" placeholder="Type to filter skills..." class="fld w-full bg-gray-50 border border-gray-200 rounded-xl pl-11 pr-4 py-2.5 text-gray-900 placeholder-gray-400 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400">
            </div>
            <div class="space-y-5 max-h-[500px] overflow-y-auto pr-2" id="skills-container">
                ...
                ?>
                        <div class="skill-category border border-gray-100 rounded-xl bg-slate-50/40 p-4 dark:border-gray-600 dark:bg-gray-700/50" data-cat="<?= sanitize_string(strtolower($cat)) ?>">
                            <div class="flex items-center gap-2 mb-3">
                                <span class="w-1.5 h-3 rounded-full bg-blue-600"></span>
                                <h3 class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider"><?= sanitize_string($cat) ?> <span class="text-[11px] text-gray-400 font-normal normal-case ml-1">(<?= count($list) ?>)</span></h3>
                            </div>
                            <div class="flex flex-wrap gap-2.5">
                                <?php foreach ($list as $skill):
                                    $isSelected = in_array($skill['id'], $prefillSkills); ?>
                                    <label class="skill-tag inline-flex items-center gap-2 border border-gray-200 rounded-xl px-3.5 py-2 text-xs text-gray-600 bg-white shadow-sm cursor-pointer select-none transition-all hover:border-blue-500 hover:bg-blue-50/10 dark:border-gray-600 dark:text-gray-300 dark:bg-gray-700 dark:hover:border-blue-500 <?= $isSelected ? 'skill-tag-active' : '' ?>" data-id="<?= $skill['id'] ?>" data-name="<?= sanitize_string(strtolower($skill['skill_name'])) ?>">
                                        <input type="checkbox" name="skills[]" value="<?= $skill['id'] ?>" class="hidden" <?= $isSelected ? 'checked' : '' ?>>
                                        <span class="w-1.5 h-1.5 rounded-full <?= $isSelected ? 'bg-blue-500 shadow shadow-blue-400' : 'bg-gray-300 dark:bg-gray-500' ?> transition-all dot-indicator"></span>
                                        <span class="font-medium text-gray-700 dark:text-gray-300"><?= sanitize_string($skill['skill_name']) ?></span>
            ...
            <div id="skills-empty" class="hidden border border-dashed border-gray-200 rounded-xl p-8 text-center bg-gray-50/50 dark:border-gray-600 dark:bg-gray-700/30">
                <i class="fas fa-search text-gray-300 text-2xl mb-2 dark:text-gray-500"></i>
        ...
        <div class="flex items-center justify-end gap-4 fade-in" style="animation-delay:.4s">
            <a href="my_jobs.php" class="px-6 py-3 border border-gray-200 hover:border-gray-300 text-gray-600 hover:text-gray-900 rounded-xl text-sm font-semibold transition-all dark:border-gray-600 dark:text-gray-400 dark:hover:text-white dark:hover:border-gray-500">Cancel</a>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('input[name="skills[]"]').forEach(function(cb) {
            cb.addEventListener('change', function() {
                ...
                if (dot) {
                    dot.className = this.checked ? 'w-1.5 h-1.5 rounded-full bg-blue-500 shadow shadow-blue-400 transition-all dot-indicator' : 'w-1.5 h-1.5 rounded-full bg-gray-300 transition-all dot-indicator'
                }
                syncSkillCount()
            })
        });
